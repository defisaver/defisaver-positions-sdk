...
export const getKingRewards = async (provider: Client, network: NetworkNumber, walletAddress: EthAddress) => {
  const data = await fetchKingRewards(walletAddress);

  const allRewardsAmount = assetAmountInEth(data.Amount, 'KING');

  const contract = UUPSViem(provider, network);
  const cumulative = await contract.read.cumulativeClaimed([walletAddress]);
  ...
  if (amountToClaim.lessThanOrEqualTo('0')) return [];
...
export const getKingRewardsBatched = async (provider: Client, network: NetworkNumber, walletAddresses: EthAddress[]) => {
