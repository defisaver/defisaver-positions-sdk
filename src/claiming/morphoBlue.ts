...
export const fetchMorphoBlueRewards = async (provider: Client,
  network: NetworkNumber,
  walletAddress: EthAddress,
): Promise<ClaimableToken[]> => {
  if (!walletAddress) return [];
  const data = await getMorphoBlueRewardsInfo(walletAddress);
    ...
    return [
      ...acc,
      {
        ...
        additionalClaimFields: {
          originalAmount: reward.claimable, // not decreased by already claimed amount - needed for merkle proofs to work
  ...
  const rewards = await Promise.all(claimableTokens.map(async (claimableToken: MorphoClaimableToken) => {
    const distributor = createViemContractFromConfigFunc('MorphoDistributor', claimableToken.additionalClaimFields.distributor)(provider, network);
    return distributor.read.claimed([walletAddress, claimableToken.tokenAddress]);
  }));

  const updatedRewards: ClaimableToken[] = claimableTokens.map((claimableToken: MorphoClaimableToken, i: number) => {
    const claimed = assetAmountInEth(rewards[i], claimableToken.underlyingSymbol);

    // eslint-disable-next-line no-param-reassign
    claimableToken.amount = new Dec(claimableToken.amount).sub(claimed).toString();
    return claimableToken;
  });

  return updatedRewards.filter((claimableToken) => new Dec(claimableToken.amount).gt(0));
};

export const fetchMorphoBlueRewardsBatched = async (
  provider: Client,
  network: NetworkNumber,
