...
export const fetchSparkAirdropRewards = async (provider: Client, network: NetworkNumber, user: EthAddress) => {
  const data = await fetchSpkAirdropRewards(user);
  ...
  if (falseResponse || emptyResponse || singleResponseWithoutAmount) return [];
  ...
  const cumulativeClaimedAmounts = await getCumulativeClaimedSpkAirdrop(
    provider,
    network,
    filteredData.map((airdropInfo: { wallet_address: any; token_address: any; epoch: any; type: any; }) => ({
      user: airdropInfo.wallet_address, tokenAddress: airdropInfo.token_address, epoch: airdropInfo.epoch, type: airdropInfo.type,
    })));

  return filteredData.map((rewardInfo: { token_address: string | undefined; amount_normalized: Dec.Value; wallet_address: any; root_hash: any; proof: any; epoch: any; type: any; }, i: string | number) => {
    const assetInfo = getAssetInfoByAddress(rewardInfo.token_address);
    return ({
      symbol: assetInfo.symbol,
      underlyingSymbol: assetInfo.symbol,
      // indexing cumulativeClaimedAmounts because we are multicalling there
      // @ts-ignore
      amount: new Dec(rewardInfo.amount_normalized).minus(cumulativeClaimedAmounts[i]).toString(),
    ...
    .filter((rewardInfo: { amount: string; }) => new Dec(rewardInfo.amount).gt('0'));
};

export const fetchSparkAirdropRewardsBatched = async (
  provider: Client,
...
export const fetchSparkRewards = async (
  ...
  console.log(rewardsPerAAsset[0]);
