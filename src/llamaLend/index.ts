...
export const getLlamaLendGlobalData = async (web3: Web3, network: NetworkNumber, selectedMarket: LlamaLendMarketData): Promise<LlamaLendGlobalMarketData> => {
  ...
  const multicallData = [
    {
      ...
      params: [getAssetInfo(debtAsset), USD_QUOTE],
    },
    {
  ...
  const debtUsdPrice = getEthAmountForDecimals(multiRes[0][0],18);
  ...
  const leftToBorrow = getEthAmountForDecimals(data.leftToBorrow, 18);
  ...
  assetsData[debtAsset] = {
    ...
    price: debtUsdPrice,
    supplyRate: data.borrowApr,
    borrowRate: data.lendApr,
    canBeSupplied: true,
    canBeBorrowed: true,
    shares,
  };
  ...
  assetsData[collAsset] = {
    ...
    price: new Dec(debtUsdPrice).mul(data.collateralPrice).toString(),
    supplyRate: '0',
    borrowRate: '0',
    canBeSupplied: true,
    canBeBorrowed: false,
