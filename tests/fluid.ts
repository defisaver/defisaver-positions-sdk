...
describe('Fluid', () => {
  ...
  const fetchUserNftIds = async (user: EthAddress, _web3: Web3) => sdk.fluid.getFluidVaultIdsForUser(_web3, NetworkNumber.Eth, user);

  const fetchMarketData = async (network: NetworkNumber, _web3: Web3) => {
    const marketData = await sdk.fluid.getFluidMarketData(_web3, network, sdk.markets.FluidMarkets(network)[sdk.FluidMainnetVersion.ETH_USDC_11]);
  ...
  it('can fetch user nft ids on Ethereum', async function () {
    this.timeout(10000);
    const nftIds = await fetchUserNftIds(testWhaleAddress, web3);
    console.log(nftIds);
  });
  ...
  //   this.timeout(10000);
  //   const nftIds = await fetchUserNftIds(testWhaleAddress, web3Arb);
  //   console.log(nftIds);
  // });
});
